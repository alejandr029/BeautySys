                    ...
                    <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                        <div
                            ...
                            @if(auth()->user()->hasRole(['admin']))
                                ...
                                <button class="crear"
                                        style="margin-right: 15px;background-color: #F2F2F2; border-color:#F2F2F2;"
                                        onclick="window.location.href='{{ route('Inventario.crearInsumo') }}'; mostrarLoader();">
                                    <a>
                                    <span style="color: #0D0D0D;">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24"
                                             height="24"><path fill="none" d="M0 0h24v24H0z"></path><path
                        ...
                        <div class="table-responsive p-0">
                            <table class="table align-items-center mb-0">
                                ...
                                <tr>
                                    ...
                                    <th class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7"
                                        style="text-align: center;">Fecha Adquisición
                                    </th>
                                    <th class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7"
                                        style="text-align: center;">Fecha Vencimiento
                                    </th>
                                    <th class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7"
                                        style="text-align: center;">Cantidad
                                    </th>
                                    <th class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7"
                                        style="text-align: center;">Estatus
                                    </th>
                                    <th class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7"
                                        style="text-align: center;">Acciones
                                    </th>
                                </tr>
                                </thead>
                                <tbody>
                                @foreach ($Insumos as $insumo)
                                    <tr>
                                        ...
                                        <td><img src="{{ $insumo->imagen }}"
                                                 class="avatar avatar-sm-new me-3 border-radius-lg"
                                                 alt="{{ $insumo->nombre }}" style="display: block; margin: auto;"></td>
                                        ...
                                        <td class="td-actions">
                                            <div role="group">
                                                <button type="button" class="btn btn-info"
                                                        style="margin:0rem 0.5rem 0.5rem 0rem; flex:none;"
                                                        onclick="window.location.href='{{ route('Inventario.vistaInsumo', ['id' => $insumo->id_insumos]) }}'; mostrarLoader();">
                                                    <i class="material-icons">visibility</i></button>
                                                @if(auth()->user()->hasRole(['admin']))
                                                    <button type="button" class="btn btn-warning"
                                                            style="margin:0rem 0.5rem 0.5rem 0rem; flex:none;"
                                                            onclick="window.location.href='{{ route('Inventario.show', ['id' => $insumo->id_insumos]) }}'; mostrarLoader();">
                                                        <i class="material-icons">edit</i></button>
                                                    <form
                                                        action="{{ route('insumos.cambiarEstado', ['id' => $insumo->id_insumos]) }}"
                                                        method="POST" onsubmit="mostrarLoader()">
                                                        @csrf
                                                        @method('get')
                    ...
                    <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                        <div
                            ...
                            @if(auth()->user()->hasRole(['admin']))
                                ...
                                <button class="crear"
                                        style="margin-right: 15px;background-color: #F2F2F2; border-color:#F2F2F2;"
                                        onclick="window.location.href='{{ route('Inventario.crearEquipo') }}'; mostrarLoader();">
                                    <a>
                                    <span style="color: #0D0D0D;">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24"
                                             height="24"><path fill="none" d="M0 0h24v24H0z"></path><path
                            ...
                            <table class="table align-items-center mb-0">
                                ...
                                @foreach ($equipoMedico as $equipo)
                                    <tr>
                                        ...
                                        <td><img src="{{ $equipo->imagen }}"
                                                 class="avatar avatar-sm-new me-3 border-radius-lg"
                                                 alt="{{ $equipo->nombre }}" style="display: block; margin: auto;"></td>
                                        ...
                                        <td class="td-actions">
                                            <div role="group">
                                                <button type="button" class="btn btn-info"
                                                        style="margin:0rem 0.5rem 0.5rem 0rem; flex:none;"
                                                        onclick="window.location.href='{{ route('Inventario.vistaequipo', ['id' => $equipo->id_equipo_medico]) }}'; mostrarLoader();"
                                                ><i class="material-icons">visibility</i></button>
                                                @if(auth()->user()->hasRole(['admin']))
                                                    <button type="button" class="btn btn-warning"
                                                            style="margin:0rem 0.5rem 0.5rem 0rem; flex:none;"
                                                            onclick="window.location.href='{{ route('Inventario.showEquipo', ['id' => $equipo->id_equipo_medico]) }}'; mostrarLoader();"
                                                    ><i class="material-icons">edit</i></button>
                                                    <form
                                                        action="{{ route('equipo.cambiarEstado', ['id' => $equipo->id_equipo_medico]) }}"
                                                        method="POST" onsubmit="mostrarLoader()">
                                                        @csrf
                                                        @method('get')
            ...
            <div class="my-custom-paginator" style="display: flex; justify-content: flex-end; margin-right: 25px;">
                ...
                <ul class="pagination">
                    ...
                    <li class="page-item {{ $equipoMedico->currentPage() === $equipoMedico->lastPage() ? 'disabled' : '' }}">
                        <a class="page-link" href="{{ $equipoMedico->url($equipoMedico->lastPage()) }}"
                           aria-label="Last" onclick="mostrarLoader()">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                                 class="bi bi-chevron-double-right" viewBox="0 0 16 16">
                                <path fill-rule="evenodd"
                                      d="M3.646 1.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1 0 .708l-6 6a.5.5 0 0 1-.708-.708L9.293 8 3.646 2.354a.5.5 0 0 1 0-.708z"/>
