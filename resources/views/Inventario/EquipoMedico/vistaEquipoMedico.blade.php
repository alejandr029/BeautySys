        ...
        <div class="col-lg-11">
          <div class="card">
            <div class="card-header">
              <h4 class="card-title">Vista del insumo con ID: {{ $equipoMedico->id_equipo_medico }} </h4>
            </div>
            <div class="card-body">
                <div class="row mb-5">
                  <div class="col-md-4">
                    <div class="input-form">
                      <input type="text" id="nombre" name="nombre" required value="{{ $equipoMedico->nombre }}" disabled>
                      <label for="nombre" class="textUser fixed-label">Nombre</label>
                    </div>
                  </div>
                  <div class="col-md-4">
                    <div class="input-form">
                      <input type="text" id="modelo" name="modelo" required value="{{ $equipoMedico->modelo }}" disabled>
                      <label for="nombre" class="textUser fixed-label">Modelo</label>
                    </div>
                  </div>
                  <div class="col-md-4">
                    <div class="input-form">
                      <input type="text" id="marca" name="marca" required value="{{ $equipoMedico->marca }}" disabled>
                      <label for="nombre" class="textUser fixed-label">marca</label>
                    </div>
                  </div>
                </div>
                <div class="row mb-5">
                  <div class="col-md-4">
                    <div class="input-form">
                      <input type="number" id="cantidad" name="cantidad" required value="{{ $equipoMedico->cantidad }}" disabled>
                      <label for="cantidad" class="textUser fixed-label">Cantidad de equipo medico</label>
                    </div>
                  </div>
                  <div class="col-md-4">
                      <div class="input-form">
                          <select id="estatus" name="id_estatus_equipo" required disabled>
                              <option value="" >Seleccionar Estatus</option>
                              @foreach ($estatus as $item)
                                  <option value="{{ $item->id_estatus_equipo }}" {{ $equipoMedico->id_estatus_equipo == $item->id_estatus_equipo ? 'selected' : '' }}>
                                      {{ $item->nombre }}
                                  </option>
                              @endforeach
                          </select>
                          <label for="estatus" class="textUser fixed-label" style="visibility: hidden">Estatus de equipo medico</label>
                      </div>
                  </div>
                    <div class="col-md-4">
                      <div class="input-form">
                          <select id="proveedor" name="id_proveedor" required disabled>
                              ...
                              @foreach ($proveedores as $proveedor)
                                  <option value="{{ $proveedor->id_proveedor }}" {{ $equipoMedico->id_proveedor == $proveedor->id_proveedor ? 'selected' : '' }}>
                ...
                <div class="row mb-5">
                  ...
                  <input id="url" class="footer" type="text" placeholder="Coloca una URL" name="imagen_url" value="{{ $equipoMedico->imagen }}" disabled>
              </div>
              ...
              <script>
                  ...
                  $('#url').on('input', function() {
                      ...
                      <div class="col-md-4">
                          ...
                          <textarea id="descripcion" name="descripcion" required disabled>{{ $equipoMedico->descripcion }}</textarea>
                          <label for="descripcion" class="textUser fixed-label">Descripción</label>
                          </div>
                      </div>
                      <div class="col-md-4">
                        <div class="checkbox-wrapper-31">
                          <input {{$equipoMedico->devolucion == 1 ? 'checked' : ''}} type="checkbox" name=devolucion disabled>
                        ...
                        <label>¿Se devolvera el equipo medico?</label>
                        </div>
                </div>
            </div>
          </div>
