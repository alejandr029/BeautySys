          ...
          <div class="card-body">
            <div class="row">
              <div class="col-lg-11">
                ...
                <form action="{{ route('Citas.update', ['id' => $cita->id_cita]) }}" method="POST" class="role-form">
                  @csrf
                  @method('PUT')
                  <!-- Campo: Hora de la cita -->
                  <div class="mb-3">
                    <label for="hora_cita" class="form-label">Hora de la cita:</label>
                    <input type="text" class="form-control input-form" id="hora_cita" name="hora_cita" value="{{ Carbon::parse($cita->hora_cita)->format('h:i A') }}">
                  </div>

                  <!-- Campo: Fecha de la cita -->
                  <div class="mb-3">
                    <label for="fecha_cita" class="form-label">Fecha de la cita:</label>
                    <input type="date" class="form-control input-form" id="fecha_cita" name="fecha_cita" value="{{ $cita->fecha_cita }}">
                  </div>
                  ...
                  <div class="mb-3">
                    ...
                    <select class="form-control" id="id_estado_cita" name="id_estado_cita">
                      ...
                      <option value="{{ $estado->id_estado_cita }}" {{ $estado->id_estado_cita == $cita->id_estado_cita ? 'selected' : '' }}>
                        {{ $estado->nombre }}
                      </option>
                      @endforeach
                    </select>
                  ...
                  <div class="mb-3">
                    ...
                    <select class="form-control" id="id_sala" name="id_sala">
                      ...
                      <option value="{{ $sala->id_sala }}" {{ $sala->id_sala == $cita->id_sala ? 'selected' : '' }}>
                  ...
                  <!-- Campo: Tipo de cita (Opciones disponibles) -->
                  <div class="mb-3">
                    <label for="id_tipo_cita" class="form-label">Tipo de cita:</label>
                    <select class="form-control" id="id_tipo_cita" name="id_tipo_cita">
                      @foreach($tipoCita as $tipoCita)
                      <option value="{{ $tipoCita->id_tipo_cita }}" {{ $tipoCita->id_tipo_cita == $cita->id_tipo_cita ? 'selected' : '' }}>
                        {{ $tipoCita->nombre }}
                      </option>
                      @endforeach
                    </select>
                  ...
                  <!-- Campo: Doctor que atenderá (Opciones disponibles) -->
                  <div class="mb-3">
                    ...
                    <select class="form-control" id="id_personal" name="id_personal">
                      ...
                      <option value="{{ $person->id_personal }}" {{ $person->id_personal == $cita->id_personal ? 'selected' : '' }}>
                  ...
                  <!-- Campo: Insumos (Opciones disponibles) -->
                  <div class="mb-3">
                    ...
                    <select class="form-control" id="id_insumo" name="id_insumo">
                      ...
                      <option value="{{ $insumo->id_insumo }}" {{ $cita->insumos && $cita->insumos->id_insumo == $insumo->id_insumo ? 'selected' : '' }}>
                        {{ $insumo->nombre }}
                      </option>
                      @endforeach
                    </select>
                  ...
                  <!-- Campo: Equipo para la consulta (Opciones disponibles) -->
                  <div class="mb-3">
                    ...
                    <select class="form-control" id="id_equipo" name="id_equipo">
                      ...
                      <option value="{{ $equipoMedico->id_equipo_medico }}" {{ $cita->equipoMedico && $cita->equipoMedico->id_equipo_medico == $equipoMedico->id_equipo_medico ? 'selected' : '' }}>
                        {{ $equipoMedico->nombre }}
                      </option>
                      @endforeach
                    </select>
